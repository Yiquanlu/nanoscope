...
def parse_arguments():
    ...
    parser.add_argument('--modality_barcode', type=str, required=True,
                        help='Modality and barcode combination in format "modality_barcode" (e.g. "H3K27ac_ACGATAGC")')
...
def process_sample_modality(args, sample, modality, downsample_ratio):
    ...
    try:
        # Construct path using modality_barcode directly
        sample_dir = os.path.join(args.output_dir, 
                                f"{sample}/{args.modality_barcode}/cellranger/outs")
        bam_file = os.path.join(sample_dir, 'possorted_bam.bam')
        
        if not os.path.exists(bam_file):
            print(f"BAM file not found for {sample}/{args.modality_barcode}, skipping")
            return None
...
def main():
    ...
    # Get modality from modality_barcode argument
    modality = args.modality_barcode.split('_')[0]
    modalities = [modality]
    ...
    for sample in samples:
        for modality in modalities:
            for ratio in args.downsample_ratios:
                # Check if sample/modality_barcode exists
                if os.path.exists(os.path.join(args.output_dir, sample, args.modality_barcode)):
